...
def track_progress(request):
    ...
    goals = SavingsGoal.objects.filter(user=request.user)
    for goal in goals:
        # Calculate the total contributions made towards the goal
        total_contributed = goal.contributions.aggregate(total=Sum('amount'))['total'] or 0
        # Add total_contributed to the goal object
        goal.total_contributed = total_contributed
        # Calculate the remaining amount needed to reach the goal
        goal.remaining_amount = goal.target_amount - total_contributed

    # Render the progress.html template with the goals data
    return render(request, 'registration/progress.html', {'goals': goals})